...
class Scheduler:
    ...
    def schedule_deployment(self) -> None:
        ...
        deployment_ids = self.__redis_client.z_range_by_score(
            self.__REDIS_KEY, "-inf", "+inf"
        )
        ...
        for deployment_id in deployment_ids:
            try:
                ...
                if deployment.status != DeploymentStatus.QUEUED:
                    logger.warning(
                        f"[Scheduler]: {deployment} is already in terminal state"
                    )
                ...
                if self.__deployment_service.has_sufficient_resources(
                    ...
                    logger.info(
                        f"[Scheduler]: {deployment} cannot be scheduled due to insufficient resources."
                    )

            except ResourceDoesNotExistsError:
                self.__redis_client.z_rem(self.__REDIS_KEY, deployment_id)
                logger.exception(
                    f"[Scheduler]: Deployment {deployment_id} does not exist."
                )
                continue
    ...
    def __allocate_resource(self, deployment: Deployment) -> None:
        ...
        logger.info(
            f"[Scheduler]: {deployment} scheduled successfully on cluster {deployment.cluster}"
        )
    ...
    def cleanup_completed_deployments(self) -> None:
        ...
        logger.info(
            "[Scheduler]: Checking for completed/failed deployments to cleanup..."
        )
        ...
        for deployment in terminals:
            try:
                self.__deployment_service.clean_up_deployment(
                    deployment_id=deployment.id, status=deployment.status
                )
                logger.info(
                    f"[Scheduler]: {deployment} is marked as {deployment.status} and resources are cleaned up."
                )
            except Exception as exception:
                logger.exception(
                    f"[Scheduler]: Failed to cleanup deployment {deployment}, Err: {exception}"
                )
                continue
